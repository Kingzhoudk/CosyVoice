...
def delete_voice_model(voice_name):
    ...
    # 检查是否为预训练音色
    if voice_name in cosyvoice.list_available_spks():
        gr.Warning("无法删除预训练音色")
        return False
        
    try:
        voice_path = f"{ROOT_DIR}/voices/{voice_name}.pt"
        if os.path.exists(voice_path):
            os.remove(voice_path)
            gr.Info(f"音色 '{voice_name}' 删除成功")
            # 刷新音色列表
            return refresh_sft_spk()
        else:
            gr.Warning("音色文件不存在")
            return False
    except Exception as e:
...
def main():
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            ...
            delete_voice_button = gr.Button("删除音色")  # 添加删除音色按钮
        ...
        delete_voice_button.click(fn=delete_voice_model, inputs=[sft_dropdown], outputs=[sft_dropdown])  # 绑定删除按钮事件
